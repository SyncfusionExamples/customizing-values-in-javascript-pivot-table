...
var pivotObj = new ej.pivotview.PivotView({
    ...
    aggregateCellInfo: function (args) {
        // Here, we verify that the current cell has a row header labeled 'France' under "FY 2015," a column header labeled "Q1," and a value field labeled "Sold."
        if ((args.row.valueSort.levelName).includes('FY 2015.France') && args.fieldName === 'Sold' && args.column.actualText === 'Q1') {
            // If it does, we check if the row belongs to the child by using level
            if (args.row.level === 2) {
                // If we multiply the value of each child member
                this.value *= args.value;
            }
            // Here, we determine if the current cell is part of the row subtotal by checking its level.
            if (args.row.level == 1) {
                if (this.value != 1) {
                    // We assign the calculated value here.
